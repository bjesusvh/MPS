...
#' }
MPS <- function(Xcand, B0, B, R, p, target = NULL, method = "kl", verbose = FALSE){
  ...
  out <- list(method = method, loss = e.loss, ranking = ranking, selected = ifelse(1:n %in% selected, TRUE, FALSE), yHat = data.frame(yHat))
  class(out) <- "MPS"
  return(out)
}
